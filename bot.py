...
def get_account_type(update: Update, context: CallbackContext) -> int:
    response = update["message"]["text"]
    context.user_data["last_search"]["account_type"] = mc.human_to_machine(response)
...
def get_time_window(update: Update, context: CallbackContext) -> int:
    response = update["message"]["text"]
    context.user_data["last_search"]["time_window"] = mc.human_to_machine(response)
...
def send_result(update: Update, context: CallbackContext) -> int:
    ...
    placeholder = update.message.reply_text(
        "*Searching...*", reply_markup=ReplyKeyboardRemove(), parse_mode="Markdown"
    )

    response = update["message"]["text"]
    context.user_data["last_search"]["match_type"] = mc.human_to_machine(response)
    ...
    if (
        context.user_data["last_search"] not in context.user_data["store"]
        and "not found" not in context.user_data["last_result"]
    ):
...
def save_player_button(update: Update, context: CallbackContext) -> None:
    ...
    callback = query["message"]["reply_markup"]["inline_keyboard"][0][0][
        "callback_data"
    ]
    query.answer()
    ...
    if callback == "save":
        context.user_data["store"].append(context.user_data["last_search"])
        ...
        context.user_data["store"].remove(context.user_data["store"][index])
...
def send_result_list(update: Update, context: CallbackContext) -> int:
    try:
        ...
        update.message.reply_text(
            "Something went wrong 😕", reply_markup=ReplyKeyboardRemove()
        )
        return ConversationHandler.END
...
def article(
    id, title, description, message_text, account_type
) -> InlineQueryResultArticle:
